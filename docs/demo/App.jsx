...
const App = () => {
  ...
  // Run real chaos against the encode service.
  const runChaos = useCallback(async () => {
    clearAllTimers();
    setToolLines([]);
    setEnvState("incident-detected");
    pushToast({ tone: "amber", msg: "Triggering chaos…", meta: "encode service" });
    try {
      const resp = await fetch("/api/chaos/trigger", { method: "POST" });
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const data = await resp.json();
      pushToast({
        tone: "red",
        msg: `Chaos unleashed · ${data.message_count} encoding requests`,
        meta: data.asset_id.slice(0, 12),
      });
    } catch (e) {
      schedule(800, () => {
        pushToast({ tone: "red", msg: "Chaos unleashed · 80 encoding requests", meta: "demo mode" });
      });
    }
  }, [clearAllTimers, pushToast, schedule]);

  // Trigger the deployed agent. The fetch will hold the connection open for the
  // full diagnose → gate → execute cycle (~30-60s). While we wait, the polling
  // tick on /api/approvals will detect the pending plan and the ApprovalGate
  // modal will open automatically.
  const [diagnosing, setDiagnosing] = useState(false);
  const runDiagnose = useCallback(async () => {
    if (diagnosing) return;
    setDiagnosing(true);
    setEnvState("agent-diagnosing");
    setToolLines([]);
    pushToast({ tone: "indigo", msg: "Diagnose · Pro 3.1 reasoning", meta: "Dynatrace MCP" });
    try {
      const resp = await fetch("/api/agent/diagnose", {
        method: "POST",
      ...
      if (!resp.ok) {
        ...
        throw new Error(`HTTP ${resp.status}: ${detail.slice(0, 80)}`);
      }
      const data = await resp.json();
      // Surface the agent's tool trail in the right-side ToolStream.
      if (Array.isArray(data.tool_calls)) {
        const d = new Date();
        const t = d.toLocaleTimeString("en-GB", { hour12: false });
        const lines = data.tool_calls
          .filter(tc => tc.phase === "request")
          .map(tc => ({
            t,
            tool: tc.tool,
            args: Object.keys(tc.args || {}).join(", "),
            result: "→ ok",
            resultTone: "ok",
          }));
        setToolLines(lines);
      }
      const tone = data.approval_status === "approved" ? "green" : "amber";
      pushToast({
        tone,
        msg: `Agent · ${data.approval_status || "done"}`,
        meta: data.audit_id ? data.audit_id.slice(0, 12) : (data.plan_id ? data.plan_id.slice(0, 8) : ""),
      });
      setEnvState(data.approval_status === "approved" ? "recovered" : "healthy");
      setDiagnosing(false);
    } catch (e) {
      // Offline / Demo presentation mode fallback
      const script = window.SF_AGENT_SCRIPT || [];
      script.forEach((step, idx) => {
        schedule(step.tAt, () => {
          const t = new Date().toLocaleTimeString("en-GB", { hour12: false });
          setToolLines(prev => [...prev, {
            t, tool: step.tool, args: step.args, result: step.result, resultTone: step.resultTone
          }]);
  ...
  const onApprove = useCallback(async () => {
    ...
    schedule(1400, () => {
      ...
      pushToast({ tone: "green", msg: "Recovered", meta: "Incident resolved successfully." });
    });
  ...
  const onReject = useCallback(async () => {
    ...
    pushToast({
      tone: (ok || !livePending) ? "neutral" : "red",
      msg: (ok || !livePending) ? "Remediation rejected." : "Reject failed",
      meta: "logged",
    });
  }, [decideLive, livePending, pushToast]);
